...
const Order = ({ match }) => {
  ...
  }, [dispatch, successPayMessage, successDeliver, refId, orderId, initialLoading]);

  useEffect(() => {
    if (success && initialLoading) {
      setInitialLoading(false);
    }
  }, [dispatch, success, initialLoading]);

  useEffect(() => {
    if (refId) {
      dispatch(payOrder(orderId));
    }
  }, [refId, dispatch, orderId]);
            ...
            <Col md={4}>
              <Card>
                <ListGroup variant="flush">
                  ...
                  {order.payment && order.payment.paymentMethod === 'COD' && !order.isPaid && (
                    <ListGroup.Item>
                      <Button variant="outlined" color="primary" fullWidth readOnly style={{ cursor: 'not-allowed' }}>
                        COD
                      </Button>
                    </ListGroup.Item>
                  )}
